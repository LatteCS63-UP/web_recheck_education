...
function page() {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        // * SysLogin
        // const response_login = await fetch(`http://10.50.9.17:5000/login/showPerson?Username=${formLogin.Email}&Password=${formLogin.Password}`)
        // const response_login = await fetch(`http://localhost:5000/login/checkEmailLogin?Email=${formLogin.Email}`)
        // const data = await response_login.json();
        // console.log(data.status_email_login)

        // const { data: session }: any = useSession()

        // console.log(session.status_email_login)
    ...
    return (
        <div className='testFormPage px-2'>
            <div className={`container bg-white ${isSignUp ? 'active' : 'active'}`} id='container'>
